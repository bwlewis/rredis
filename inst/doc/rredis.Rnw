...
command pipelining,
structured value types, server replication, data expiration, clustering,
multicast-like publish/subscribe, and it's very fast.
...
The {\tt redisWatch} function  function provides a check and set style
